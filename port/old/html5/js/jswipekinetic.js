...
(function($) {
	...
	*/
	...
	//	var _iTop = (parseInt($element_.css('marginTop')) + oDelta_.lastY);
		var _iTop = ($element_.offset().top + oDelta_.lastY);
	...
	*/
	...
	//		var _oValues = _calculateValues(_$element,_oDelta,true);
			...
			$cubes.each(function() {
				// kinetic scrolling
				var _oValues = _calculateValues(jQuery(this),_oDelta,true);

				jQuery(this)
					.css('-webkit-transition',['webkitTransform ', _oValues.iDuration,'s ease-out'].join(''))
					.css('-webkit-transition',['transform ', _oValues.iDuration,'s ease-out'].join(''))
					.css({
						'webkitTransform': ['rotateX(370deg) translateZ(',_oValues.iTop,'px) translateY(',0,'px)'].join(''),
						'transform': ['rotateX(370deg) translateZ(',_oValues.iTop,'px) translateY(',0,'px)'].join('')
					});
			});
	};
	...
	//		var _oValues = _calculateValues(_$element,_oDelta,false);
			...
			$cubes.each(function() {
				var _oValues = _calculateValues(jQuery(this),_oDelta,false);
	...
	$.fn.jSwipeKinetic.touchend = function(event_) {
		...
		// elasitc snap to bounding box if a swipemove was the last movement
 		...
 		//	var _oValues = _calculateValues(_$element,{lastX:0,lastY:0}, true);
			...
			$cubes.each(function() {
				var _oValues = _calculateValues(jQuery(this),{lastX:0,lastY:0}, true);

				jQuery(this)
					.css('-webkit-transition',['webkitTransform ', (_oValues.iDuration === 1)  ? 0 : _oValues.iDuration ,'s ease-out'].join(''))
					.css('-webkit-transition',['transform ', (_oValues.iDuration === 1)  ? 0 : _oValues.iDuration ,'s ease-out'].join(''))
					.css({
						'webkitTransform': ['rotateX(370deg) translateZ(',_oValues.iTop,'px) translateY(',0,'px)'].join(''),
						'transform': ['rotateX(370deg) translateZ(',_oValues.iTop,'px) translateY(',0,'px)'].join('')
					});
